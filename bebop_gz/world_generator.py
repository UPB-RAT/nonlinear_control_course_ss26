...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Error: num_drones must be an integer")
            print("Usage: python3 generate_bebop.py [num_drones=X]")
            sys.exit(1)

    if num_drones < 1:
        print("Error: num_drones must be at least 1")
        sys.exit(1)

    if num_drones > 1:
        print("Warning: only 1 drone is supported — forcing num_drones=1")
        num_drones = 1
...
def generate_bebop_files(num_drones):
    ...
    print(f"  SDF world : {world_path}")
    print(f"  YAML config: {yaml_path}")
    print(f"\nConfigured {num_drones} Bebop drone(s)")
...
def generate_sdf_world(num_drones):
    ...
    for i in range(1, num_drones + 1):
        ...
        # SetPose plugin
        set_pose = ET.SubElement(include, 'plugin',
                                 name='ignition::gazebo::systems::SetPosePlugin',
                                 filename='libSetPosePlugin.so')
        ET.SubElement(set_pose, 'robotNamespace').text = bebop_name
        ET.SubElement(set_pose, 'topic').text          = f'{bebop_name}/set_pose'

        # PosePublisher plugin
        pose_pub = ET.SubElement(include, 'plugin',
                                 name='ignition::gazebo::systems::RobotPosePublisher',
                                 filename='libRobotPosePublisher.so')
        ET.SubElement(pose_pub, 'robotNamespace').text = bebop_name
        ET.SubElement(pose_pub, 'topic').text          = f'{bebop_name}/pose'
...
def generate_yaml_config(num_drones):
    ...
    for i in range(1, num_drones + 1):
        ...
        drone_config = f"""# Drone {i} bridge configuration
